...
import deliveryPriceTool from './deliveryPriceTool'
...
class LocationTool {
	...
	*/
	public async setLocationFullState(location: LocationType) {
		deliveryPriceTool.setDeliveryPriceByLocation(location)
		const new_location = { ...location, ...this.location_detail }
		const address = await this.getAddressString(new_location)
		store.dispatch(setLocationAction({ location: new_location, address }))
	}
	...
	*/
	public async findMe(): Promise<LocationType> {
		...
		const location = await useLoadingPromise(this.getLocation())
