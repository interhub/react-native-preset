...
export const {
  ...
  initialState: state,
  reducers: {
    setTableInfoAction(state, {payload: table_info}: PayloadAction<TableInfoItemType[]>) {
      state.table_info = table_info
    },
    setLoadingAction(state, {payload: loading}: PayloadAction<boolean>) {
      state.loading = loading
    },
    setErrorStringAction(state, {payload: error}: PayloadAction<string>) {
      state.error = error
    },
  },
})
