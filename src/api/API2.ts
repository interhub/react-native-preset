...
import {setErrorStringAction, setHistoryAction, setLoadingAction, setMenuAction, setSalesAction, setTokenAction, setUserAction, setAppStatusAction} from './../store/actions'
...
class API extends ToolsAPI {
  ...
  authPhone = async ({code, phone_number}: {phone_number: string; code: string}): Promise<AxiosResponse<AuthDataType>> => {
    const res = await useLoadingPromise(
      this.server.post<AuthDataType>('/api/auth', {
        phone_number,
  ...
  registration = async ({name, email, phone_number, code}: {name: string; email: string; phone_number: string; code: string}): Promise<AxiosResponse<AuthDataType>> => {
    const res = await useLoadingPromise(
      this.server.post<AuthDataType>('/api/users', {
        phone_number,
        verification_code: code,
        email,
        name,
  ...
  getMenuSet = async (): Promise<AxiosResponse<ResType<{menu?: MenuType}>>> => {
    ...
    if (menu) this.dispatch(setMenuAction(menu))
    return res
  }
  ...
  getSalesSet = async () => {
    ...
    if (promotions) this.dispatch(setSalesAction(promotions))
    return res
  }
  ...
  changeUser = async (info: ChangeUserInfoType): Promise<AxiosResponse<ResType>> => {
    ...
    if (!res?.data?.success) this.setErrorDisplay(res?.data.error_message)
    return res
  }

  payRequest = async (): Promise<AxiosResponse<ResType<{payment_url?: string}>>> => {
    const {order_info, address} = store.getState()
    return await useLoadingPromise(
      this.server.post<ResType<{payment_url?: string}>>(
        '/api/orders',
        {
          order_info,
          address,
  ...
  cancelOrder = async ({id}: {id: number}) => {
    ...
    if (res?.data?.history) this.dispatch(setHistoryAction(res.data.history))
    return res
  }
